...
void AppManager::update(SystemServices& s) {
  ...
  if(ev.type != TouchType::None) _active->handleTouch(s, ev);
  if(s.requestHome) { s.requestHome=false; open(s,"springboard"); return; }
...
void AppManager::render(SystemServices& s, bool full) {
  ...
  s.board->drawText(260,10,String("GPS:")+(g.valid?"fix":"search"),0,1);
